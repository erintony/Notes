...
## 添加locale.js转换文件
```
/** 当前环境下要转换成的语言 t: 繁体中文， s: 简体中文 , 默认简体中文*/
let zh_choose = "s";

/** 浏览器的默认语言 navigator.language */
let zh_browserLang = "";
/** 是否开启简体中文 -> 繁体中文转换 */
const zh_autoLang_t = true;
/** 是否开启繁体中文 -> 简体中文转换 */
const zh_autoLang_s = true;

/** 繁体中文环境检测正则 */
const zh_langReg_t = /^zh-tw|zh-hk$/i;
/** 简体中文环境检测正则 */
const zh_langReg_s = /^zh-cn$/i;
...
function getStorage(name) {
  let result = name ? sessionStorage.getItem(name) : "";
  return result || "";
}
 ...
 * @param el： 当前dom元素
 */
function zh_tran(go, el) {
  if (go) zh_choose = go;
  setStorage("zh_choose", zh_choose);
  zh_tranBody(el);
}
 ...
 */
function zh_getLang() {
  if (getStorage("zh_choose")) {
    zh_choose = getStorage("zh_choose");
    return true;
  }
...
function zh_init(el) {
  zh_getLang();
  zh_tran(zh_choose, el);
}

export default zh_init;
...
##  在main.js中添加全局混入
```
import zh_init from "./locale";
Vue.mixin({
  mounted: function () {
    this.$nextTick(() => zh_init(this.$el));
  },
  updated() {
    this.$nextTick(() => zh_init(this.$el));
  },
